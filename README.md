...
## ⚠️ v4 Audit Correction (2026-04-20): Majority baseline reveals null is deeper

Phase 9 (all 14 sub-phases) compared model to random + keyword but missed
the **majority-class baseline**. Recomputation on GoEmotions validation n=500:

| Task | Majority baseline | Keyword | Model |
|------|------------------:|--------:|------:|
| 10-way (always-CARE) | **27.6%** | 28.0% | 19.2% (**-8.4% below majority**) |
| 6-way Ekman (always-joy) | **53.2%** | 36.4% | 36.4% (**both -16.8% below**) |

The Phase 9.4 "null" is actually "both methods fail to exceed trivial prior
on 10-way; model fails worse." The Phase 9.12 "tie at 6-way" (celebrated as
positive) is **tie at catastrophic failure below always-joy baseline**.

**Lesion specificity claims (Phase 9.6, 9.10) lack statistical support**:
Fisher exact test p-values for all 7 tested lesions ≥ 0.1. "4/10 specificity"
is descriptive-only, not statistically significant at tested n (3-11 per class).

**Binary valence (v4 verification)**: keyword 66.8% / model 66.6% / majority 63.5%.
Both methods exceed majority baseline by ~3% at binary — the ONLY granularity
where either works above trivial prior. Model and keyword tied.

See `docs/audit_v4.md` for full revision.

## Known Limitations (originally 2026-04-19, updated post-v4)
...
1. **Rate-matching validation は predictive power 不足** — behavioral prediction
   **empirically confirmed as null at statistical significance** (Phase 9):
   ...
   - **Dimensional affect regression**: first partial positive — model beats keyword on
     ...
     by hand-coded emotion→V/A weight table, not circuit dynamics.
     Details: `docs/phase9_dimensional_va.md`
   - **Hybrid control (Phase 9.9)**: apply V/A weights to keyword hits directly
     (bypass simulation). **Hybrid beats model dramatically on arousal** (Pearson +0.272
     vs -0.019; MAE 0.219 vs 0.450); valence tied. The neural simulation actively
     **coarsens** arousal signal that keyword counts already carry. Details: `docs/phase9_hybrid_control.md`
   - **Population-level lesion (Phase 9.10)**: silence neurons directly (tonic=-10) instead
     of just input drives. RAGE/SEEKING/SADNESS show clean specificity (accuracy → 0%).
     **SEEKING specificity is NEW**, previously masked by input-lesion readout bias.
     FEAR shows multi-pathway redundancy (consistent with LeDoux 2000). 4/10 emotions
     now have empirical specificity evidence. Details: `docs/phase9_pop_lesion.md`
   - **Reference ceiling (Phase 9.11)**: Gemini 2.5-flash 24% / keyword 23% / model 17.5% /
     LR trained 12% / random 10.5%. **LLM ceiling is barely above keyword** — task is
     inherently hard for all methods. Model sits between LR and keyword, offers no
     unique value but is not catastrophically bad. Details: `docs/phase9_ceiling.md`
   - **Coarse-grained Ekman 6 (Phase 9.12)**: at 6-way granularity (anger/disgust/fear/
     joy/sadness/surprise), **model 36.4% = keyword 36.4% (tied)**. The 10-way null
     disappears at coarse granularity — model captures broad affect categories
     equivalently to keyword. Combined with circuit specificity, this supports a
     viable "interpretable coarse emotion classifier" positioning. Details:
     `docs/phase9_coarse_grained.md`
   - **Top-k ranking (Phase 9.13)**: on 10-way, model trails keyword at top-1 by 5.8%
     but only by 2.4% at top-5. Ranking quality is similar; argmax commitment is
     specifically where model fails. True emotion is in model's top-3 41.2% of the
     time vs keyword 44.0%. Details: `docs/phase9_topk.md`
   - **Multi-label (Phase 9.14)**: GoEmotions multi-label instances (16% of n=500),
     P/R/F1/hit @k metrics. **F1@5 model 0.201 vs keyword 0.208** (Δ=-0.007, tied).
     Gap narrows dramatically as k grows. Model's ranking is nearly equivalent to
     keyword's except at strict argmax. Details: `docs/phase9_multilabel.md`
