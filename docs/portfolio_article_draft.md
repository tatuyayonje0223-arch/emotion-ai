...
Phase 6-7-8 は継続するが、positioning を shift:
- **3d (Education/Demo)**: interactive web UI で脳領域活動を可視化 (本記事と同時に公開)
- **3e (Portfolio)**: 本記事 — 試行錯誤を含む honest writeup
- **3c (B2B Interpretable AI)**: 長期 — behavioral prediction validation + 標準 simulator 移行検討

「EU AI Act 2026 高リスク emotion recognition 適合」は solo dev では非現実的と判定。
小規模 education/portfolio/interpretability 領域で価値を出す方針に pivot。
